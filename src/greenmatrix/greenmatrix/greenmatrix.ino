...
void getGlypth_tomThumb5(int charID, uint8_t* glyph)
{
  // 4x6
  // 3x5 usable
...
void loop() {
  ...
  for(uint8_t i=0; i<NUM_ANODES; ++i) {
    ...
    for(uint8_t j=0; j<3; ++j) {
      uint8_t b = g0[j] & (1 << (4-i));
      digitalWrite(CATHODES[j], b ? LOW : HIGH);      
    }
    for(uint8_t j=0; j<3; ++j) {
      uint8_t b = g1[j] & (1 << (4-i));
      digitalWrite(CATHODES[j+4], b ? LOW : HIGH);      
    }
    delay(5);
    digitalWrite(ANODES[i], LOW);
  }
