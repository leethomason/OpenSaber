    ...
    const UIRenderData* data)
{
    ...
    if (mode == UIMode::NORMAL) {
        for (int r = 0; r < 8; ++r) {
            ...
            if (r < p)
                ren->DrawRect(fx.getInt(), 28 - r * 4, BAR_W, 3, WHITE);
            else
                ren->DrawRect(fx.getInt(), 28 - r * 4 + 1, BAR_W, 1, WHITE);
            //if (r >= p) 
            //    ren->DrawRect(fx.getInt() + 1, 28 - r * 4, BAR_W-2, 2, BLACK);
        }
    }
    ...
    // Audio
    {
        for (int r = 0; r < 8; ++r) {
            ...
            Fixed115 fx = Fixed115(8, 10) * d * d;
            if (r / 2 < data->volume)
                ren->DrawRect(W - fx.getInt() - BAR_W, 28 - r * 4, BAR_W, 3, WHITE);
            else
                ren->DrawRect(W - fx.getInt() - BAR_W, 28 - r * 4 + 1, BAR_W, 1, WHITE);
        }
    }
    // Color indicator
    {
