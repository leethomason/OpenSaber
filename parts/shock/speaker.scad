...
//intersection()
{
	difference() 
	{
		// main piece	
		union() {
			tube(TUBE_COUPLE_H + TRAN, TUBE_COUPLE_D/2, D_INNER/2);
			translate([0, 0, TUBE_COUPLE_H]) {
				shelf(TUBE_COUPLE_D	/2, TRAN, SPKR_METAL_D/2);
				translate([0, 0, TRAN]) {
					tube(SPKR_METAL_H - TRAN, SPKR_METAL_D/2, D_INNER/2);
					translate([0, 0, SPKR_METAL_H - TRAN]) {
						tube(SPKR_PLASTIC_H, SPKR_PLASTIC_D/2, D_INNER/2);
					}
				}
			}
		}
		...
		for(r=[0:1]) {
            ...
            S2 = 4;
			rotate([0,0,r*180]) {
				translate([D_INNER	/2 - S, -S/2, TUBE_COUPLE_H + 2]) {
