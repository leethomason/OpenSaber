...
module buttressKnockout(battery, trough, clip, wiring, h) 
{	
	// Battery
	if (battery) {
					...
					h=H_BUTTRESS)
{
	...
	{
		cylinder(h=h, d=D_AFT);	
		buttressKnockout(battery, trough, clip, wiring, h);
	}
}
...
module archButtress()
{
	difference() {
	    intersection() {
	    	translate([0, 0, -20]) cylinder(h=40, d=D_AFT);

	        rotate([0, 90, 0]) translate([0, Y_MC + H_MC, -20]) 
	        	arch(16, 1.00, 50, 2.2);
	        	//arch(16, 0.95, 50, 2.2);
	    }
	    translate([0,0,-10]) {
	    	...
	    	clip = true;
	    	wiring = false;
			buttressKnockout(battery, trough, clip, wiring, 20);
	    }
	}
}
...
buttress();
translate([0, 0, H_BUTTRESS]) intersection() {
	cylinder(h=100, d=D_AFT);
	union() {
		for(i=[0:3]) {
			translate([0, 0, i*8]) archButtress();	
		}	
	}
}

*for(i=[0:3]) {
	translate([0, 0, i*9]) buttress(trough=W_MC, clip=true);
}